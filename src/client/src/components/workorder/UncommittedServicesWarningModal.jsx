...
/**
 * Pre-invoice warning that surfaces any service packages or inventory-pulled parts
 * still in draft state. The user can commit them inline or proceed to invoice anyway.
 */
const UncommittedServicesWarningModal = ({
  ...
  const hasPackages = uncommittedPackages.length > 0;
  const hasParts = uncommittedParts.length > 0;
  const total = uncommittedPackages.length + uncommittedParts.length;

  const headerTitle = hasPackages && hasParts
    ? 'Uncommitted Items Found'
    : hasPackages
      ? 'Uncommitted Services Found'
      : 'Uncommitted Parts Found';
          ...
          <div className="overflow-y-auto flex-1 min-h-0 p-5">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <div className="font-semibold text-yellow-900 mb-1">
                    {total} item{total === 1 ? '' : 's'} not yet pulled from inventory
                  </div>
                  <p className="text-sm text-yellow-800">
                    The following have been added to this work order but have NOT been pulled from inventory yet.
            ...
            {hasParts && (
              <div className="space-y-3">
                <h4 className="font-semibold text-gray-900 text-sm mb-3">
                  Uncommitted Parts ({uncommittedParts.length}):
                </h4>
                {uncommittedParts.map((part, idx) => (
                  <div
                    key={`part-${idx}`}
                    className="border-2 border-dashed border-yellow-300 bg-yellow-50/30 rounded-lg p-4"
                  >
                    ...
                    <button
                      onClick={() => onCommitPart(part.partIndex)}
                      className="w-full py-2 px-4 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium text-sm transition-colors"
                    >
                      <i className="fas fa-arrow-down mr-2"></i>
                      Pull from Inventory Now
            ...
            <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1 text-sm text-blue-800">
                  ...
                  <p>
                    It deducts the item from stock and marks it as committed.
                    Only committed items appear on invoices and affect inventory levels.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="px-5 py-4 border-t border-gray-200 bg-gray-50">
            <div className="flex gap-3">
              ...
              >
                ...
                Generate Invoice Without These
              </button>
            </div>
            <p className="text-xs text-center text-gray-500 mt-2">
              Uncommitted items will not be billed on this invoice
            </p>
          </div>
        </div>
      </div>
