...
const InvoiceGenerator = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Temporarily using a random number for invoice count until backend service is confirmed
  ...
  const saveInvoice = async () => {
    ...
    try {
      // Use the imported InvoiceService
      if (InvoiceService && typeof InvoiceService.createInvoice === 'function') {
        await InvoiceService.createInvoice(finalInvoice); 
        alert('Invoice saved successfully!');
      } else {
        console.error("InvoiceService.createInvoice is not a function or InvoiceService is not available.");
        alert('Error: InvoiceService is not properly configured. See console for details.');
        // Fallback or detailed error logging
        console.log("Attempted to save (but service was unavailable):", finalInvoice);
      }
      
      // Update Work Order Status if a work order was selected
      if (selectedWorkOrder && WorkOrderService && typeof WorkOrderService.updateWorkOrderStatus === 'function') {
        // Assuming updateWorkOrderStatus exists and is correctly imported/available
        // This might need adjustment based on actual WorkOrderService capabilities
        await WorkOrderService.updateWorkOrderStatus(selectedWorkOrder._id, 'Invoiced');
        console.log(`Work order ${selectedWorkOrder._id} status updated to Invoiced.`);
      } else if (selectedWorkOrder) {
        console.warn("WorkOrderService.updateWorkOrderStatus is not available. Status not updated.");
      }

    } catch (err) {
      console.error("Error saving invoice or updating work order status:", err);
      setError(`Failed to save invoice: ${err.message}. Check console for more details.`);
      // Provide more specific feedback if possible
      if (err.response && err.response.data && err.response.data.message) {
        setError(`Failed to save invoice: ${err.response.data.message}`);
      }
    }
  };
