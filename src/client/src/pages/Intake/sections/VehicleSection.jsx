...
const VehicleSection = ({ customer, onSaved, onError }) => {
  ...
  const handleVinDecode = async (vin, setFieldValue) => {
    ...
    try {
      ...
      if (result.success) {
        if (result.data.year) setFieldValue('year', result.data.year);
        if (result.data.make) setFieldValue('make', result.data.make);
        if (result.data.model) setFieldValue('model', result.data.model);
        setVinError(null);
      } else {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-500">
          Vehicles for {customer?.name} ({vehicles.length})
        </h4>
        <Button
      ...
      {initialLoading ? (
        ...
        <div className="text-center py-6 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          <i className="fas fa-car text-2xl text-gray-400 mb-2"></i>
          <p className="text-gray-600 text-sm">No vehicles found for this customer.</p>
          <p className="text-xs text-gray-500">Click "New Vehicle" to add one.</p>
        </div>
      ) : (
