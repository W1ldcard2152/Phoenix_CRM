...
- **Notes can be written against a specific job** instead of only against the work order as a whole. A note filed under a job prints inside that job's block on the printed work order, quote and invoice, so on a multi-job repair the customer can tell which repair a note refers to. Notes written in the work order's own Notes card still print at the bottom of the document as before. Removing a job asks what to do with its notes: keep them on the work order, still labeled with the job they were written under, or delete them along with the job.
- **Order Stock page** (Supplies → Order Stock): everything at or below its reorder point, grouped by vendor so each group is one order to place, with out-of-stock items first and the product link on the row. Each vendor's group copies on its own, with a suggested quantity in whole purchase units — a 3-quart gap on a 5-quart jug is one jug, not 0.6 — and what it is expected to cost. The page never changes stock; stock goes up when it physically arrives, through Receive stock.
...
- **Automatic backups.** Every night at 2 AM the shop's data is backed up to the shop's own storage. The first backup of each month is kept for good and the rest for 35 days, and each one is locked for 30 days so it can't be deleted — by mistake or by anyone with access. If the system was asleep or updating at 2 AM, it catches up shortly after it starts.
- **A Backups page for admins** (cog menu → Backups): see when the shop was last backed up, back up on demand, download a copy to keep yourself, and restore any saved backup or a file you kept. The page warns you if the shop's storage isn't set up to protect backups or to recover deleted photos.
- **Restoring is safe to undo.** Before any restore, a backup of the shop as it is right now is taken automatically — if that fails, nothing is restored — and it appears as a "Before restore" backup you can restore to undo. Saving is paused for everyone while a restore runs, photos and files are never touched, and the admin running it always keeps their account. A restore refuses a damaged file, or a backup from a different shop, before changing anything.
